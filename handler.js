...
let knight = require('knights-canvas')
...
let image = await new knight.Welcome2()
    .setAvatar(pp)
    .setUsername(await this.getName(user)) 
    .setBg("https://caliph71.xyz/static/image/bg.png") 
    .setGroupname(groupMetadata.subject) 
    .setMember(groupMetadata.participants.length)
    .toAttachment();
 this.sendFile(jid, image.toBuffer(), 'welcome.png', text, { 
key: {
fromMe: false,
...
let image = await new knight.Goodbye2()
    .setAvatar(pp)
    .setUsername(await this.getName(user)) 
    .setBg("https://caliph71.xyz/static/image/bg2.jpg") 
    .setMember(groupMetadata.participants.length)
    .toAttachment();
 this.sendFile(jid, image.toBuffer(), 'goodbye.png', text, { 
key: {
fromMe: false,
participant: '0@s.whatsapp.net',
remoteJid: 'status@broadcast'
...
global.dfail = (type, m, conn) => {
  let msg = {
    ...
    unreg: '── 「 NOT REGISTERED 」 ──\nSilakan Register Terlebih Dahulu Sebelum Menggunakan Bot. Cara Register Cukup Dengan Command *#daftar*',
